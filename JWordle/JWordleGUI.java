...
public class JWordleGUI extends JComponent implements KeyListener, MouseListener{
   
   //============================================================================================================
   //==========================   FUNCTIONS FOR STUDENT TO CALL IN JWordleLogic.java   ==========================
   //============================================================================================================
   ...
   public static char[] getSecretWord(){
      return Arrays.copyOf(JWordleGUI.secretWord, JWordleLogic.MAX_COLS);
   }
   ...
   public static void setGridLetter(int row, int col, char letter){
      if (!isValidRowCol(row, col))
         throw new IllegalArgumentException("Invalid cell specified, row: " + row + ", col: " + col);
      if (letter != JWordleLogic.NULL_CHAR){
   ...
   public static char getGridLetter(int row, int col){
      if (!isValidRowCol(row, col))
         throw new IllegalArgumentException("Invalid cell specified, row: " + row + ", col: " + col);        
   ...
   public static void endGame(boolean didPlayerWin){
      if (didPlayerWin)
   ...
   public static void wiggleGrid(int row){
...
//************************************************************************************    
   ...
   private static final int GAMEOVER_FONT_SIZE = 30;  
   ...
   private static final String DEBUGTXT_VISIBLE = "[WORD = ";       
   private static final String DEBUGTXT_HARDCODE = "[HARDCODED USED]";       
   private static final String DEBUGTXT_VALIDATION = "[ANY GUESS VALID]"; 
...
//*************     Class Variables    **************   
   ...
   private static JWordleGUI canvas = new JWordleGUI();
   ...
   //Tracks the secret word the user is trying to guess, generated by JWordleLogic
   ...
   private static void initWindow(){                
      ...
      //Allows game to use mouse/keyPressed methods implemented in JWordleGUI.java
   ...
   private static void initKeyboard(){
      ...
      keyboard.put(JWordleLogic.ENTER_KEY, enter);
      ...
      keyboard.put(JWordleLogic.BACKSPACE_KEY, backspace);
   ...
   private void drawDebugText(Graphics g){        
      ...
      String debugText = "";
      if (GameLauncher.DEBUG_SHOW_SECRET_WORD)
   ...
   public void keyPressed(KeyEvent event) { 
      ...
      if (ALL_LETTERS.indexOf(keyChar) >= 0 || keyChar == JWordleLogic.ENTER_KEY || keyChar == JWordleLogic.BACKSPACE_KEY)
         JWordleLogic.keyPressed(keyChar);
      repaint();
   }
   ...
   static class LetterCell extends JPanel{
      ...
      private LetterCell(int x, int y){
         ...
         this.x = x;
         this.y = y; 
         this.letter = JWordleLogic.NULL_CHAR;
      }
   ...
   private static void validateNormalizeSecretWord(){
      if (JWordleGUI.secretWord == null)
         throw new IllegalStateException("ERROR! null char array returned from initGame() for secret word!");      
      else if (JWordleGUI.secretWord.length != JWordleLogic.MAX_COLS)
         throw new IllegalStateException("ERROR! Secret word array incorrect length!  Expected: " + JWordleLogic.MAX_COLS +", got: " + JWordleGUI.secretWord.length);
      for (int i = 0; i < JWordleGUI.secretWord.length; i++){
         JWordleGUI.secretWord[i] = charToUpperCase(JWordleGUI.secretWord[i]);
         if (ALL_LETTERS.indexOf(JWordleGUI.secretWord[i]) < 0)
            throw new IllegalStateException("ERROR! Secret word contains non alphabetic character: " + Arrays.toString(JWordleGUI.secretWord));
      }
   }
   ...
   public static void launchGame() {              
      ...
      JWordleGUI.secretWord = JWordleLogic.initGame();
